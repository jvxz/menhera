import { colors, MenheraThemes } from './colors'
...
export function createThemeHelpers({ color, soft = false, black = false }: GetThemeOptions) {
  ...
  const v = (key: keyof typeof MenheraThemes, op = '') => {
    let obj = black
      ? MenheraThemes[`black${capitalize(key)}` as keyof typeof MenheraThemes] || MenheraThemes[key]
      : soft
        ? MenheraThemes[`soft${capitalize(key)}` as keyof typeof MenheraThemes] || MenheraThemes[key]
        : MenheraThemes[key]
